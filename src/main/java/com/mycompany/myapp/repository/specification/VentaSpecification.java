...
public interface VentaSpecification extends JpaSpecificationExecutor<Venta> {
    public static Specification<Venta> searchingParam(String filter) {
        return new Specification<Venta>() {
            private static final long serialVersionUID = 1L;

            public Predicate toPredicate(Root<Venta> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
                query.distinct(true);
                ...
                Join<Venta, Coche> venta = root.join("venta", JoinType.LEFT);
